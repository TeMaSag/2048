$(document).ready(function () {
  ...
  var flagActionCell = false;//no actions in cells
  var flagEndGame = false;//game over
  var flagGameWinner = false;//game winner
  ...
  for (i = 0; i < $('#Game').children().length; i++) {
    var column1 = [];
    for (j = 0; j < $('#Game').children().length; j++) {
      var column2 = row[j][i];
  ...
  function initWindow() {
    ...
    for (let i = 1; i <= SIDE_SIZE; i++) {
      ...
      //$("#str"+i).append("<div class=clearfix>");
    }
  }

  function randCell(elems) {
    var randCell = Math.floor(Math.random() * elems.length);
    return randCell;
  }

  function checkWinner() {
    if (!flagGameWinner) {
      for (i = 1; i <= SIDE_SIZE * SIDE_SIZE; i++) {
        if (+$("#cell" + i).text() == 2048) {
          alert('Winer');
          flagGameWinner = true;
        }
  ...
  function gameOver() {
    var checkEmptyCell = createMasEmptyCell();
  ...
  function fillCell() {
    var emptyCell = createMasEmptyCell();
    ...
    //
    var InitValIndex = randCell(emptyCell);
  ...
  function actionRightUp(el) {
    ...
    for (let i = el.length - 1; i > 0; i--) {
      var prevCell = $(el[i - 1]).text();
      var currSell = $(el[i]).text();
  ...
  function actionLeftDown(el) {
    ...
    for (let i = 0; i <= el.length - 1; i++) {
      var currCell = $(el[i + 1]).text();
      var followCell = $(el[i]).text();
